...
def generate_certificate_pdf(user_name, member_id, course_name, completion_date, certificate_code):
    buffer = io.BytesIO()
    ...
    c.setFillColor(HexColor('#2C3E50'))
    c.drawCentredString(width / 2, height - 240, "for successfully completing the course")
    
    c.setFont("Helvetica-Bold", 20)
    c.setFillColor(HexColor('#2980B9'))
    c.drawCentredString(width / 2, height - 280, course_name)
    ...
    c.setFillColor(HexColor('#2C3E50'))
    c.drawString(100, height - 460, "_________________________")
    c.drawString(100, height - 478, "Program Director")
    
    c.drawString(width - 250, height - 460, "_________________________")
    c.drawString(width - 250, height - 478, "Executive Director")
